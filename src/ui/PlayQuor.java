...
public class PlayQuor{
    ...
    public static void main(String[] args) throws InterruptedException{
        String[] options = {"Two player game", "Four player game","Exit"};
        ...
        while (true){
            ...
            n = JOptionPane.showOptionDialog(GameBoardWithButtons.contentPane, 
                    "Select opponent location","Network Game?",
                    JOptionPane.YES_NO_CANCEL_OPTION,JOptionPane.QUESTION_MESSAGE,null, options,options[0]);
            if (n == 1){
                networkGame = true;
                if(numPlay == 2){
                String address = JOptionPane.showInputDialog("Input the player's network address", "hostname or IP");
                NetworkClient network = new NetworkClient(address);
                }else{ // numPlay is 4
                    String address = JOptionPane.showInputDialog("Input the addresses of 3 players separated by spaces", "ex. hostname0 hostname1 hostname2");
                    String address1;
                    String address2;
                    String address3;
                    Scanner sc = new Scanner(address);
                    while(sc.hasNext()){
                        //find a way to transfer these or something
